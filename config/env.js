...
// Grab NODE_ENV and AURA* environment variables and prepare them to be
// injected into the application via DefinePlugin in Webpack configuration.
const AURA = /^AURA_/i;

function getClientEnvironment(target, options) {
  const raw = Object.keys(process.env)
    .filter(key => AURA.test(key))
    .reduce(
      (env, key) => {
      ...
      },
      {
        ...
        AURA_ASSETS_MANIFEST: paths.appManifest,
        ...
        // It's useful if you're running AURA in a non-localhost container. Ends in a /
