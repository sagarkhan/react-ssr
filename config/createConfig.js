...
module.exports = (
  ...
  const getStyleLoaders = (cssOptions = {}, preProcessor) => {
    const { localIdentName, modules, importLoaders } = cssOptions;
    const loaders = IS_NODE ?
      // Style-loader does not work in Node.js without some crazy
      // magic. Luckily we just need css-loader.
      [
        {
          loader: modules ? require.resolve('css-loader/locals') : require.resolve('css-loader'),
          options: cssOptions,
        },
      ]
      : IS_DEV ?
    ...
    if (preProcessor) {
      ...
      if(preProcessor === 'sass-loader') {
        loaders.push({
          ...
          options: {
            resources: `${paths.appSrc}/client/variables.scss`,
          },
        });
      }
    }
  ...
  let config = {
    ...
    module: {
      ...
      rules: [
        ...
        {
          test: /\.scss$/,
          exclude: [paths.appBuild, /\.module\.scss$/],
          use: getStyleLoaders({
            importLoaders: 2
          }, 'sass-loader')
        },
  ...
  // Apply plugins, if they are present in webpack.config.js
